...
with open(filename2, "r") as fichier:
    chi_final_data2 = json.load(fichier)
with open(filename3, "r") as fichier:
...
def plot_Teff(lines_data, chi_final_data, size_police=12, save=None):
    ...
    ax.scatter(x_vals, y_vals, color="gray", label=f"4000 K",marker='o', linewidths=0.03)
    ax.scatter(x_vals, y_vals2, color="gray", label=f"bacchus 4307 K",marker='x', linewidths=0.5)
    ax.scatter(x_vals, y_vals3, color="gray", label=f"bacchus 4258 K",marker='+', linewidths=1.2)
    ...
    ax.plot(X_line, Y_line, color='indianred', label=f'pente : {slope:.2f} pour 4000 K et log g : 1.0')
    ...
    ax.plot(X_line, Y_line2, color='green', label=f'pente : {slope2:.2f} pour 4307 K et log g : 2.29')
    ...
    ax.plot(X_line, Y_line3, color='orange', label=f'pente : {slope3:.2f} pour 4258 K et log g : 2.04')
...
# plot_Teff(raie_Fe, chi_final_data, size_police=14, save="comparaison_modèles")
...
def analyse_abu(raies,save=None, abu_to_plot=None, minimisation=None):
    chi_final={}
    variable="Fe"
