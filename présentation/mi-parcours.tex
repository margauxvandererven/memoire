...
\begin{frame}[fragile]{Abondances C, N, O}
    \begin{columns}
            ...
            \begin{table}[h!]
              ...
              \begin{center}
                  \begin{tabular}{c|cccc}
                    ...
                    Final&8.31$\pm$ 0.01 & 7.88 & 7.84 $\pm$ 0.02 & 12 \\
                  \end{tabular}
              \end{center} 
              \end{table}
        ...
        \begin{tikzpicture}[>=stealth,auto, node distance=1cm]
            \node[block, fill=UnccOrange!30] (block1) {OH};
            \node[block, right=of block1, fill=UnccGold] (block2) {O};
            \node[block, below=of block2, fill=UnccOrange!30] (block3) {CO};
            \node[block, right=of block3, fill=UnccGold] (block4) {C};
            \node[block, below=of block4,fill=BrickRed!30] (block5) {$^{12}$C/$^{13}$C};
            \node[block, right=of block4,fill=UnccOrange!30] (block6) {CN};
            \node[block, below=of block6, fill=UnccGold] (block7) {N};
            \node[block, below=of block7, fill=BrickRed!30] (block8) {$^{12}$C/$^{13}$C};
            
            \draw[->] (block1) -- (block2) ;
            \draw[->] (block2) -- (block3);
            \draw[->] (block3) -- (block4);
            \draw[->] (block4.south) -- (block5.north) ;
            \draw[->] (block5) -- (block4);
            \draw[->] (block4) -- (block6);
            \draw[->] (block6) -- (block7);
            \draw[->] (block7) -- (block8);
            ...
            \draw[->] (block4.north)-| ++(0,2.35) -| (block1.north);
        \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
